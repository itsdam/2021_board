			...
			<table class="table table-striped"
					...
					<tbody style="background-color: white;">
            			...
            			%>
							...
							<a href="boardUpdate.jsp?BbsID=<%=bbsID%>&BoardID=<%=list.get(i).getBoardID()%>&BoardContent=<%=list.get(i).getBoardContent()%>" class="button2">수정</a>
							<a onclick="return confirm('정말로 삭제하시겠습니까?')" href="boardDeleteAction.jsp?BoardID=<%=list.get(i).getBoardID()%>&BbsID=<%=bbsID%>"class="button2">삭제</a>
							</td>
						...
						<td colspan = "3" style= "text-align: left;" ><textarea type="text" class="form-control"
								placeholder="댓글을 입력하세요." name="boardContent" maxlength="2048" style= "background-color: white;"></textarea></td>
				...
				%>
					...
					<a onclick="return confirm('정말로 삭제하시겠습니까?')" href="deleteAction.jsp?bbsID=<%= bbsID %>" class="button">삭제</a>
				<%
					}
				%>
